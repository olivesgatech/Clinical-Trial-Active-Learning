...
class cResNet(nn.Module):
    def __init__(self, type=18, num_classes=10, pretrained=False):
        ...
        if type == 18:
            self.backbone = models.resnet18(weights=wt)
        elif type == 34:
            self.backbone = models.resnet34(weights=wt)
        elif type == 50:
            self.backbone = models.resnet50(weights=wt)
        elif type == 101:
            self.backbone = models.resnet101(weights=wt)
        elif type == 152:
            self.backbone = models.resnet152(weights=wt)
        else:
    ...
    def forward(self, x):
        ...
        #print('penultimate layer shape: ', self.penultimate_layer.shape)
